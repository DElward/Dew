...
struct js_reserved_words {
    ...
    { "arguments"     , JSKW_ARGUMENTS       , JSW_KW  , JSKWI_KEYWORD   },
    ...
    { "else"          , JSKW_ELSE            , JSW_KW  , JSKWI_RW        },
    ...
    { "while"         , JSKW_WHILE           , JSW_KW  , JSKWI_RW        },
...
/***************************************************************/
static short jrun_find_keyword(const char * kwnam)
{
/*
**
*/
    ...
    if (ix < nb_reserved_words && !strcmp(kwnam, js_reserved_word_list[ix].jrw_name)) {
        kw = js_reserved_word_list[ix].jrw_rw;
    } else {
        kw = 0;
    }
...
*/
    ...
    while (!done) {
        ...
        if (!jstat) {
            do {
                jchar = jtok_get_char(jx, jstr, jstrix);
                if (jchar != '\\') {
                    if (jchar >= 128 || JTOK_TOKE_CHAR(jchar)) {
                        jtok_append_token(jtok, jchar);
                    } else {
                        done = 1;
                    }
    ...
    if (jstat) {
        ...
        if (jtok->jtok_tlen == 0 || !JTOK_FIRST_TOKE_CHAR(jtok->jtok_text[0])) {
...
*/
    ...
    if (!jchar) {
        ...
        do {
            jtok_append_token(jtok, jchar);
            jchar = jtok_get_char(jx, jstr, jstrix);
        } while (JTOK_TOKE_CHAR(jchar));
    } else {
        done = 0;
        while (!done) {
            jtok_append_token(jtok, jchar);
            if (!JTOK_TOKE_CHAR(jchar)) {
        ...
        if (jtok->jtok_typ == JSW_ID) {
            jtok->jtok_kw = jrun_find_keyword(jtok->jtok_text);
            if (jtok->jtok_kw) {
                jtok->jtok_typ = JSW_KW;
            }
        }
    }

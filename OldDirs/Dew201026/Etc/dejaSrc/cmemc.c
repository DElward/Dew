...
/*  fprintf(repfnum, "ptr=%8lx", meminfo->ptr); */
    ...
    get_abuf(abuf, TMIN(12,meminfo->numbytes), (void*)(meminfo->ptr));

    fprintf(repfnum, "%-36.36s", dispfname(meminfo->fname, 44));
...
/*@*/static meminfo_t * searchhashtable(hashtable_t * hashtable,
    ...
    for (hashix = 0; hashix < hashtable->hashsize; hashix++) {
        ...
        while (hashptr != NULL) {
            ...
            if ((intr)ptr >= meminfo->ptr &&
                (intr)ptr < meminfo->ptr + meminfo->numbytes) {
                return (meminfo);
            }
            hashptr = hashptr->nexthash;
        }

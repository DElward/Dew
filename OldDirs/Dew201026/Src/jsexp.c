...
*/
    ...
    while (!jstat && (*depth) > 0) {
        (*depth)--;
        if (xstack[depth].xs_info == XS_INFO_UNOP) {
        ...
        switch (state) {
            ...
            case JEXPST_BINARY_OPERATOR:
                ...
                if (xlx < 0) {
                    ...
                    jstat = jexp_eval_stack(xstack, &depth, jexp_exp_list[xlx].jxe_bin_precedence);
                    xstack[depth].xs_xlx  = -1;
                    xstack[depth].xs_info = XS_INFO_OPERAND;
                    xstack[depth].xs_jvv  = NULL;
                    depth++;

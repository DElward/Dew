...
/***************************************************************/
static int jexp_binop_exp(struct jrunexec * jx,
    struct jvarvalue * jvv1,
    struct jvarvalue * jvv2)
{
/*
...
struct jexp_exp_rec { /* jxe_ */
    ...
    { 1, JSPU_PLUS              , 0, jexp_binop_add     , jexp_binop_pos    },
    { 1, JSPU_MINUS             , 0, jexp_binop_sub     , jexp_binop_neg    },
    { 0, 0                      , 0, NULL               , NULL              }
};
...
*/
    ...
    for (ii = depth - 1; ii >= 0; ii--) {
        ...
        switch (xstack[ii].xs_info) {
            ...
            case XS_INFO_BINOP_OPERAND:
                strcpy(info, "Binary op");
                jexp_quick_tostring(jx, &(xstack[ii].xs_jvv), valstr, sizeof(valstr));
                break;
...
*/
    ...
    jexp_print_stack(jx, "Stack pre-eval",xstack, *depth, bin_precedence);
            ...
            switch (xstack[*depth - 1].xs_info) {
                ...
                case XS_INFO_OPERAND:
                    while (!jstat && *depth > 1 &&
                        ...
                        } else if (jexp_exp_list[xlx].jxe_bin_precedence < bin_precedence) {
                            done = 1;
                        } else {
                    ...
                    if (!jstat && !bin_precedence && *depth > 1 &&
                           xstack[*depth - 2].xs_info == XS_INFO_OPEN_PAREN) {
                        (*depth)--;
                        COPY_JVARVALUE(&(xstack[*depth - 1].xs_jvv), &(xstack[*depth].xs_jvv));
                    }
                    done = 1;
                    break;
                ...
                case XS_INFO_BINOP_OPERAND:
                    ...
                    if (*depth < 2) {
                        jstat = jrun_set_error(jx, JSERR_INTERNAL_ERROR,
                            "Invalid stack depth on binary op. Found: %d", *depth);
                    } else if (xlx < 0) {
                        jstat = jrun_set_error(jx, JSERR_INTERNAL_ERROR,
                            "Invalid operator on binary op. Found: %d", xlx);
                    } else if (jexp_exp_list[xlx].jxe_bin_precedence < bin_precedence ||
                               (jexp_exp_list[xlx].jxe_bin_precedence == bin_precedence &&
                                jexp_exp_list[xlx].jxe_rtol_associativity)) {
                        done = 1;
                    } else {
                        (*depth)--;
                        binop = jexp_exp_list[xlx].jxe_binop;
                        if (binop) jstat = binop(jx, &(xstack[*depth - 1].xs_jvv), &(xstack[*depth].xs_jvv));
                    }
                    break;
        ...
        switch (state) {
            case JEXPST_FIRST_OPERAND:
                if (xlx >= 0 && jexp_exp_list[xlx].jxe_unop) {
                    ...
                    jstat = jrun_next_token(jx, pjtok);
                    xlx = GET_XLX(*pjtok);
                } else if (depth > 0 && xstack[depth-1].xs_info == XS_INFO_BINOP) {
                    INIT_JVARVALUE(&(xstack[depth-1].xs_jvv));
                    jstat = jexp_eval_primary(jx, pjtok, &(xstack[depth-1].xs_jvv));
                    xstack[depth-1].xs_info = XS_INFO_BINOP_OPERAND;
                    xlx = GET_XLX(*pjtok);

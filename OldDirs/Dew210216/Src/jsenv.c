...
/***************************************************************/
int jrun_pop_xstat_with_restore(struct jrunexec * jx, int xstat)
{
/*
** 02/01/2021
**
        ...
        if (!jstat) {
            ...
            if (next_jtok.jtok_kw == JSPU_EQ) {
                if (!jvv) {
                    ...
                    jstat = jvar_insert_new_jvarvalue(jx, (*pjtok)->jtok_text, jvv);
                }
...
*/
    ...
    jstat = jrun_push_jfuncstate(jx);   /* main */
...
#if DEBUG_XSTACK & 4
            printf("================================================================\n");
            printf("4 jrun_exec_block() %-8s toke=%-6s, stack=",(is_active?"active":"inactive"), jtok->jtok_text);
...
#if DEBUG_XSTACK & 8
            printf("================================================================\n");
            printf("8 jrun_exec_block() %-8s toke=%-6s, stack=",(is_active?"active":"inactive"), jtok->jtok_text);
            jrun_print_xstack(jx);
#endif
            if (is_active) {

...
*/
    ...
    jvar = jx->jx_globals;
    jvsr.jvsr_jx     = jx;
    jvsr.jvsr_vflags = vflags;
    while (!done && jvar) {
        done = dbtsi_scan(jvar->jvar_jvarvalue_dbt, &jvsr, jrun_debug_show_jvarrec);
        if (vflags & 1) {
            //jvar = jvar->jvar_prev;
        //} else {
            jvar = NULL;
...
*/
    ...
    if (!strcmp(toke, "allvars")) {
        ...
        jstat = jrun_exec_debug_show_vars(jx, 1);
    } else if (!strcmp(toke, "vars")) {
        printf("Current variables:\n");
        jstat = jrun_exec_debug_show_vars(jx, 0);
    }
...
*/
    ...
    if (!strcmp(toke, "debug")) {
        while (!jstat && dbgstr[ix]) {
            get_toke_char(dbgstr, &ix, toke, TOKSZ, GETTOKE_FLAGS_DEFAULT);
            if (!strcmp(toke, "show")) {
                jstat = jrun_exec_debug_show(jx, dbgstr, &ix);
            }
            if (!jstat) {
...
*/
    ...
    jstat = jrun_push_jfuncstate(jx, fjvv);
    
    //jvar = jvar_new_jvarrec();

    //jvar_push_vars(jx, jvar); /* create new variable space */

    if (!jstat) jstat = jrun_next_token(jx, &jtok);

    if (!jstat) jstat = jrun_exec_block(jx, &jtok);

    if (jstat == JERR_RETURN) {
        jstat = 0;
        vjvv = jrun_get_return_jvv(jx);
...
*/
    ...
    if (!jstat) {
        ...
        if (!jvar || !jcx) {
            ...
            jstat = jvar_new_ad_hoc_variable(jx, jvar, jcx,

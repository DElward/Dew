...
/***************************************************************/
struct jcontext * jvar_new_jcontext(struct jrunexec * jx, struct jvarrec * jvar)
{
/*
** 03/03/2021
*/
    ...
    for (ii = 0; ii < jvar->jvar_nconsts; ii++) {
        COPY_JVARVALUE(jcx->jcx_ajvv + jvar->jvar_avix[ii], jvar->jvar_aconsts);
    }
...
/***************************************************************/
struct jcontext * jvar_get_global_jcontext(struct jrunexec * jx, struct jvarrec * jvar)
{
/*
** 03/03/2021
*/
    ...
    struct jvarvalue * jvv)
{
    ...
    jcx = jvar_get_global_jcontext(jx, jvar);
...
*/
    ...
    if (vix_parent < 0) {
        ...
        jvar_new_local_const(jvvf_parent->jvvf_vars, jvv, vix_parent);
    }
...
*/
    ...
    if (tgt_jvv) {
        ...
        jvar_new_local_const(jvar, tgt_jvv, vix);
    }
    ...
    if (jx->jx_njfs) {
        ...
        if (fjvv) {
            if (fjvv->jvv_dtype == JVV_DTYPE_FUNCTION) {
                (*pjvar) = fjvv->jvv_val.jvv_val_function->jvvf_vars;
            } else if (fjvv->jvv_dtype == JVV_DTYPE_INTERNAL_METHOD) {
                (*pjvar) = fjvv->jvv_val.jvv_int_method.jvvim_cjvv->jvv_val.jvv_jvvi->jvvi_jvar;
            }
        }
    } else {
        (*pjvar) = jvar_get_global_jvarrec(jx);
        (*pjcx) = jvar_get_global_jcontext(jx, (*pjvar));
    }
}
/***************************************************************/

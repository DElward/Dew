...
#define MT_MEM  1   /* Multi-threaded memory */
...
#endif /* UNUSED */
/***************************************************************/
static void * find_block_space(struct block_info * bi, intr numbytes) {

    register int found;
    register intr size;
    register char * start_ix;
    register intr remainder;
    register intr memsize;
    register char * head;
    void * ptr;
...
*/
    ...
    if (found) {
        ...
        if (remainder > MSIZE2) {
            LVAL(head)                    = -(MTYPE)memsize;
            LVAL(head + memsize + MSIZE)  = -(MTYPE)memsize;
            LVAL(head + memsize + MSIZE2) =  (MTYPE)remainder;
            LVAL(head + size + MSIZE)     =  (MTYPE)remainder;
        } else {
            LVAL(head)                    = -(MTYPE)size;
            LVAL(head + size + MSIZE)     = -(MTYPE)size;
        }
        ptr = head + MSIZE;
        bi->blockix = head;
    } else {
...
/***************************************************************/
static struct block_info * new_t1_block(intr blocksize) {
...
** Align blockptr
*/
    while ((intr)bi->blockptr & ALIGNM1) {
        bi->blockptr++;
        bsize--;
    }
    ...
    LVAL(bi->blockix)                   = (MTYPE)size;
    LVAL(bi->blockptr + bsize - MSIZE2) = (MTYPE)size;
...
/***************************************************************/
static void * search_blocks(struct heap_info * hi, intr size) {
...
/***************************************************************/
static int make_new_block(struct heap_info * hi, intr size) {
/*
...
void T1free(void * ptr) {
    ...
    size = tail - head - MSIZE;
    LVAL(head) = (MTYPE)size;
    LVAL(tail) = (MTYPE)size;
}
/***************************************************************/
void * T1calloc(intr n, intr s) {
...
/***************************************************************/
void * T1realloc(void * p, intr size) {
...
static intr totbytes;
static intr totrepow2;
static intr totresame;
static intr totrenew;
static intr hiwater;
...
#define hash(value) (MTYPE)(((value)<0?-(value):(value)) % (hashtable->hashsize))
...
/***************************************************************/
/*@*/void out_of_memory(char * fname, int linenum, intr itemsize) {
...
/***************************************************************/
/*@*/static void inithashtable(hashtable_t * hashtable, intr hashsize)
{
        int hashix;
...
/***************************************************************/
/*@*/static hashtable_t * newhashtable(intr hashsize) {
...
typedef
        struct {
                ...
                intr    linenum;
                intr    numbytes;
                intr    numresame; /* Realloc returns same ptr */
                intr    numrenew;  /* Realloc returns new ptr */
#ifdef CMEM_CRBY
...
/***************************************************************/
/*@*/static intr check_heap_area(unsigned char * p, intr size) {
...
/*@*/void check_heap(char* fname, int linenum) {
    ...
    static intr rate = HEAP_CHECK_RATE;
...
*/
    ...
    meminfo->ptr = (intr) ptr;
    meminfo->fname = (char*) MALLOC(strlen(fname)+1);
    if (!meminfo->fname)
        out_of_memory(__FILE__,__LINE__, strlen(fname)+1);
    strcpy(meminfo->fname, fname);
    meminfo->linenum = linenum;
    meminfo->numbytes = numbytes;
    meminfo->numresame = 0;
    meminfo->numrenew = 0;
#ifdef CMEM_CRBY
    meminfo->crby     = 1;
...
/*@*/void TFree(void* ptr, char* fname, int linenum) {
    ...
    hashrec = findhashtable(mhashtable, (intr) ptr);
    if (!hashrec) {
        ...
        *((char*)0) = 0;  /* Cause blowout */
        ...
        /* MEM_CLEAR: Added 04/01/2018 */
        memset((void*)(meminfo->ptr), '~', meminfo->numbytes);
    ...
    if (!hashrec)
        ...
        if (rptr == ptr) {
          ...
          totresame++;
            ...
            delhashtable(mhashtable, hashrec);
            meminfo->ptr = (intr) rptr;
            meminfo->numrenew++;
            totrenew++;
            addhashtable(mhashtable, meminfo->ptr, meminfo);
          }
        totbytes += numbytes - meminfo->numbytes;
        if (totbytes > hiwater) {
#if HEAP_WATCHING
            if (hiwater >> 10 != totbytes >> 10) {
                printf ("CMEMC: GOT %d KB\n", totbytes >> 10);
...
/***************************************************************/
static void get_abuf(char * abuf, int nbytes, void* data) {

    char * dptr = (char*)data;
    int ii;
    int bad_ptr;

    /* bad_ptr = IsBadReadPtr(data, 1); */
    bad_ptr = 0;

    if (!bad_ptr) {
        for (ii = 0; ii < nbytes; ii++) {
            if (dptr[ii] < ' ' || dptr[ii] > '~') abuf[ii] = '.';
            else                                  abuf[ii] = dptr[ii];
        }
        abuf[ii] = '\0';
    } else {
        strncpy(abuf, "<Bad ptr>", nbytes);
    }
}
/***************************************************************/
static char * dispfname(const char * fnam, int fnammax) {
    
    int fnlen;
    int ii;
    int fl;
    static char outnam[128];

    if (fnammax >= sizeof(outnam)) fnammax = sizeof(outnam) - 1;

    fnlen = (int)strlen(fnam);
    if (fnlen <= fnammax) {
        strcpy(outnam, fnam);
    } else {
        ii = fnlen;
        ...
        fl = fnlen - ii;
        if (fl + 3 >= fnammax) {
            strcpy(outnam, fnam + (fnlen - fnammax));
        } else {
            ii = fnammax - (fl + 3);
            memcpy(outnam, fnam, ii);
            outnam[ii] = '.';
            outnam[ii + 1] = '.';
            strcpy(outnam + ii + 2, fnam + (fnlen - (fl + 1)));
        }
    }
...
static void printmeminfo(void* mi) {
    ...
    if (!hdpr) {
        fprintf(repfnum, "%-36.36s", "File Name");
        fprintf(repfnum, "%6s", "Line");
        fprintf(repfnum, "%6s", "Bytes");
        fprintf(repfnum, "%18s","Ptr");
        fprintf(repfnum, " %s","Data");
...
/*  fprintf(repfnum, "ptr=%8lx", meminfo->ptr); */
    ...
    get_abuf(abuf, (int)TMIN(16,meminfo->numbytes), (void*)(meminfo->ptr));

    fprintf(repfnum, "%-36.36s", dispfname(meminfo->fname, 36));
    fprintf(repfnum, "%6ld", meminfo->linenum);
    fprintf(repfnum, "%6ld", meminfo->numbytes);
    fprintf(repfnum, "%18s", pbuf);
    fprintf(repfnum, " %s", abuf);
...
/*@*/static meminfo_t * searchhashtable(hashtable_t * hashtable,
    ...
    for (hashix = 0; hashix < hashtable->hashsize; hashix++) {
        ...
        while (hashptr != NULL) {
            ...
            if ((uintr)ptr >= meminfo->ptr &&
                (uintr)ptr < meminfo->ptr + meminfo->numbytes) {
                return (meminfo);
            }
            hashptr = hashptr->nexthash;
        }
...
char * malloced_by(void * ptr, char * buf) {
    ...
    if (!ptr) {
        ...
        if (!hashrec) {
            ...
            if (!meminfo) {
                sprintf(buf, "Pointer (0x%lx) not malloc'd", (intr)ptr);
            } else {
                sprintf(buf,
      "Pointer (0x%lx) is byte %ld of %ld bytes malloc'd by %s line %ld",
                (intr)ptr, (intr)ptr - meminfo->ptr, meminfo->numbytes,
                meminfo->fname, meminfo->linenum);
            }
        } else {
            meminfo = hashrec->blockptr;
            sprintf(buf, "Pointer (0x%lx) malloc'd by %s line %ld",
                (intr)ptr, meminfo->fname, meminfo->linenum);
        }
    }
...
/***************************************************************/
/*@*/void out_of_memory(intr itemsize) {

...
*/
    ...
    if (fjvv->jvv_dtype == JVV_DTYPE_FUNCTION) {
        ...
        jx->jx_jfs[jx->jx_njfs]->jfs_jcx = jrun_create_function_context(jx,
            fjvv->jvv_val.jvv_val_function->jvvf_vars, outer_jcx);
        jrun_set_current_jxc(jx, &(jvvf->jvvf_begin_jxc));
    } else if (fjvv && fjvv->jvv_dtype == JVV_DTYPE_INTERNAL_METHOD) {
        jx->jx_jfs[jx->jx_njfs]->jfs_jcx = jrun_create_function_context(jx,
            fjvv->jvv_val.jvv_int_method.jvvim_cjvv->jvv_val.jvv_jvvi->jvvi_jvar, outer_jcx);
    } else {
...
*/
    ...
    if (!fjvv) {
        ...
        jstat = jexp_call_function(jx, fjvv, NULL, global_jcx, &rtnjvv);
    } else {
        jstat = jrun_set_error(jx, JSERR_INVALID_FUNCTION,

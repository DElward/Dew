...
*/
    ...
    if (jstat) jvar_free_jvarvalue_function(jvvf);
...
*/
    ...
    if (!jstat && (fjvv->jvv_val.jvv_val_function->jvvf_flags & JVVF_FLAG_RUN_TO_EOF)) {
        jstat = jrun_push_xstat(jx, XSTAT_ACTIVE_BLOCK);
    }

    jstat = jrun_push_jfuncstate(jx, fjvv, outer_jcx);  /* sets cursor */
...
//  #endif
    ...
    } else if (jstat == JERR_EOF && (fjvv->jvv_val.jvv_val_function->jvvf_flags & JVVF_FLAG_RUN_TO_EOF)) {
...
*/
    ...
    if (mjvv->jvv_dtype == JVV_DTYPE_LVAL) {
        fjvv = mjvv->jvv_val.jvv_lval.jvvv_lval;
    ...
    if (fjvv->jvv_dtype == JVV_DTYPE_INTERNAL_METHOD) {
        jstat = jexp_internal_method_call(jx, fjvv, jvvargs, rtnjvv);
    } else if (fjvv->jvv_dtype == JVV_DTYPE_FUNCTION) {
        //jstat = jrun_next_token(jx, pjtok);
        jstat = jexp_call_function(jx, pjtok, fjvv, jvvargs, jcx, rtnjvv);
    } else {
        jstat = jrun_set_error(jx, JSERR_EXP_METHOD,
            "Expecting function or method name.");
    }

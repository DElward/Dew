    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_LVAL:
            ...
            if (vflags & JPR_FLAG_SHOW_LVALUE) {
                jpr_jvarvalue_tostring(prbuf, prlen, prmax, jvv->jvv_val.jvv_lval.jvvv_lval, vflags);
            }
            break;
    ...
    do {
        ...
        if (pvix) {
            if ((*pvix) < 0 || (*pvix) >= jcx->jcx_njvv) {
                ...
                if (jcx->jcx_ajvv[*pvix])
                    append_chars(&prbuf, &prlen, &prmax, jcx->jcx_ajvv[*pvix]->jvv_sn, 4);
                else
                    append_chars(&prbuf, &prlen, &prmax, "none", 4);
#endif
                append_printf(&prbuf, &prlen, &prmax, " %-16s - ", kval);

...
#if 0 && defined(WIN32)
    #include <float.h>
    #define FP_ISQNAN(n)    (_fpclass(n) == _FPCLASS_QNAN)
    #define FP_ISSNAN(n)    (_fpclass(n) == _FPCLASS_SNAN)
    #define FP_ISNINF(n)    (_fpclass(n) == _FPCLASS_NINF)
    #define FP_ISPINF(n)    (_fpclass(n) == _FPCLASS_PINF)
    #define FP_ISNEG0(n)    (_fpclass(n) == _FPCLASS_NZ)
#else
...
#if PRINTF_EXTENSIONS
#ifdef WIN32
void spf_win_get_message(long errCode, char * errMsg, long errMsgSize) {
...
/***************************************************************/
#else
void spf_ux_get_message(long errCode, char * errMsg, long errMsgSize) {

    strncpy(errMsg, strerror(errCode), errMsgSize);
}
#endif
/***************************************************************/
void spf_get_message(long errCode, char * errMsg, long errMsgSize) {

    int mlen;

#ifdef WIN32
    spf_win_get_message(errCode, errMsg, errMsgSize);
#else
    spf_ux_get_message(errCode, errMsg, errMsgSize);
#endif
...
char * get_os_message(long errCode) {
    ...
    spf_get_message(errCode, msgbuf, msglen);

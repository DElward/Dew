...
*/
    ...
    jstat = jpr_jvarvalue_tostring(jx, &prbuf, &prlen, &prmax, jvv,
        JPR_FLAG_SHOW_TYPE | JPR_FLAGS_ALL_TYPES);
    if (jstat) {
...
*/
    ...
    jstat = jexp_eval_stack(jx, xstack, depth, &rtnflags, JX_DOT_PREC, JXE_OPFLAG_OPEN_BRACKET);
    if (!jstat) jstat = jrun_next_token(jx, pjtok);
    INIT_JVARVALUE(&jvvparms);
    if (!jstat) {
        jstat = jexp_eval(jx, pjtok, &jvvparms);
...
#ifdef OLD_WAY      /* Fixed 201/06/2022 */

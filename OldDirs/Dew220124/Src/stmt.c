    ...
    while (!jstat) {
        ...
        if (is_active) {
            ...
            if (!cmd_func && !(cmd_flags & JCR_CMDFLAG_BAD_COMMAND)) {
                cmd_flags = JCR_CMDFLAG_POP_COMPLETE | JCR_CMDFLAG_UPDATE_XSTAT;
                cmd_func = jrun_exec_implicit_stmt;
            }
        ...
        if (!is_active) {
            if ((*pjtok)->jtok_kw > 0 && (*pjtok)->jtok_kw < JSPU_ZZ_LAST) {
                ...
                cmd_flags = 0;
                cmd_func = jrun_exec_inactive_stmt;
            }
        }
        ...
        if (!jstat) {
            if (cmd_func) {
                ...
                if (!jstat) jstat = jstat = cmd_func(jx, pjtok);
            } else {
                if (!is_active) {

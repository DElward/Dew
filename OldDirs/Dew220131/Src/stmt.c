...
*/
    ...
    switch ((*pjtok)->jtok_kw) {
        ...
        case JSKW_VAR:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jexp_evaluate_inactive(jx, pjtok, 0);
            break;
...
*/
    ...
    while (!jstat && !done) {
        switch ((*pjtok)->jtok_kw) {
            ...
            default:
                jstat = jrun_exec_inactive_stmt(jx, pjtok, 0);
                //jstat = jrun_next_token(jx, pjtok);
                break;
        }
    }
...
*/
    ...
    jstat = jexp_evaluate(jx, pjtok, &jvv);
    if (!jstat) {
        vflags = JPR_FLAG_SHOW_VARS | JPR_FLAG_SHOW_LVALUE | JPR_FLAG_SHOW_OBJECTS | JPR_FLAG_TOSTRING;
...
*/
        ...
        case JSKW_CONST:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_const_stmt(jx, pjtok, jcmdflags);
            break;
...
*/
        ...
        case JSKW_FUNCTION:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_function_stmt(jx, pjtok, jcmdflags);
            break;
        ...
        case JSKW_LET:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_let_stmt(jx, pjtok, jcmdflags);
            break;

        case JSKW_RETURN:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_return_stmt(jx, pjtok, jcmdflags);
            break;

        case JSKW_THROW:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_throw_stmt(jx, pjtok, jcmdflags);
            break;

        case JSKW_TRY:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_try_stmt(jx, pjtok, jcmdflags);
            break;
        ...
        case JSKW_VAR:
            jstat = jrun_next_token(jx, pjtok);
            if (!jstat) jstat = jrun_exec_var_stmt(jx, pjtok, jcmdflags);
            break;
        ...
        default:
            jstat = jrun_exec_implicit_stmt(jx, pjtok, jcmdflags);
        ...
        if ((*pjtok)->jtok_kw == JSPU_OPEN_BRACE) {
            ...
            if (!is_active) {
                ...
                if (XSTAT_XSTATE(jx) == XSTAT_INACTIVE_TRY) {
                    if (!jstat) jstat = jrun_exec_inactive_catch(jx, pjtok, jcmdflags);
                } else {
                    if (!jstat) jstat = jrun_exec_catch_stmt(jx, pjtok, 0);
                }
            } else if (XSTAT_XSTATE(jx) != XSTAT_TRY_COMPLETE) {
            ...
            if (is_active) {
                ...
                is_active = XSTAT_IS_ACTIVE(jx);
                if (!is_active) {                               
                    jstat = jrun_exec_inactive_stmt(jx, pjtok, jcmdflags);
                } else {
                    jstat = jrun_exec_active_stmt(jx, pjtok, jcmdflags);
                }
            } else {
                jstat = jrun_exec_inactive_stmt(jx, pjtok, jcmdflags);
            }
...
*/
    ...
    if ((*pjtok)->jtok_kw == JSPU_SEMICOLON) {
        ...
        jstat = jexp_evaluate_inactive(jx, pjtok, jcmdflags);
    }
...
/* JSKW_FUNCTION          */ { 3, jrun_exec_function_stmt       , 1, jrun_exec_function_stmt        },
    ...
    while (!jstat) {
        ...
        if (!jstat) {
            if (cmd_func) {
                ...
                if (!jstat) {
                    jstat = jstat = cmd_func(jx, pjtok, jcmdflags);
                    if (jstat > 0) {
                        jstat = jrun_error_recovery(jx, pjtok, jstat);
                    }
                }

...
/***************************************************************/
void jvar_new_local_const(struct jvarrec * jvar, struct jvarvalue * jvv, int vix, int const_jcxflags)
{
/*
** 03/03/2021
*/
    if (jvar->jvar_nconsts == jvar->jvar_xconsts) {
        ...
        jvar->jvar_const_jcxflags =
            Realloc(jvar->jvar_const_jcxflags, int, jvar->jvar_xconsts);
    }
    jvar->jvar_avix[jvar->jvar_nconsts] = vix;
    COPY_JVARVALUE(jvar->jvar_aconsts + jvar->jvar_nconsts, jvv);
    jvar->jvar_const_jcxflags[jvar->jvar_nconsts] = const_jcxflags;
    jvar->jvar_nconsts += 1;
}
/***************************************************************/
#if 0
...
*/
    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_DYNAMIC :
            if (jvv->jvv_val.jvv_val_dynamic.jvvy_free_proc) {
                (jvv->jvv_val.jvv_val_dynamic.jvvy_free_proc)(jvv->jvv_val.jvv_val_dynamic.jvvy_this_ptr);
            }
            jvv->jvv_dtype = JVV_DTYPE_NONE;
            break;
...
*/
    ...
    jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_UNSUPPORTED_ASSIGN,
        "Store to Objects not supported yet.");
...
*/
    ...
    if (vix < 0) {
        ...
        jvar_new_local_const(cjvv->jvv_val.jvv_jvvi->jvvi_jvar, mjvv, vix, jcxflags);
    }
...
*/
    ...
    if (fjvv_child->jvv_dtype != JVV_DTYPE_FUNCTION ||
        ...
        pvix = jvar_find_in_jvarrec(jti_brace->jti_val.jti_jvar, 
            fjvv_child->jvv_val.jvv_val_function->jvvf_func_name);
        if (!pvix) {
            if (skip_flags & SKIP_FLAG_HOIST) {
                vix = jvar_insert_into_jvarrec(jti_brace->jti_val.jti_jvar, 
                    fjvv_child->jvv_val.jvv_val_function->jvvf_func_name);
                fjvv_child->jvv_val.jvv_val_function->jvvf_vix = vix;

                jvar_new_local_const(jti_brace->jti_val.jti_jvar, fjvv_child, vix, jcxflags);
            }
        } else {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERROBJ_SCRIPT_SYNTAX_ERROR,
                "Duplicate function name. Found: '%s'",
...
*/
    ...
    if (tgt_jvv) {
        ...
        if (jvar_is_jvv_const(jvv)) {
            jvar_new_local_const(jcx->jcx_jvar, jvv, vix, jcxflags);
        }
    }

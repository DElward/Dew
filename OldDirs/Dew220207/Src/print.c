...
*/
    ...
    while (ii < jvva->jvva_nvals) {
        ...
        if ((vflags & JPR_FLAG_TOSTRING) && jvva->jvva_avals[ii].jvv_dtype == JVV_DTYPE_NONE) {
            append_charval(prbuf, prlen, prmax, "…");
            do {
                ii++;
...
*/
    ...
    if (cjvv->jvv_dtype == JVV_DTYPE_INTERNAL_OBJECT) {
        mjvv = jvar_int_object_member(jx, cjvv, JVV_INTERNAL_METHOD_toString, &injcxflags);
    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_LVAL:
            ...
            if (vflags & JPR_FLAG_SHOW_LVALUE) {
                jstat = jexp_eval_rval((struct jrunexec *)jx, jvv, &jvvtmp);
                if (!jstat) {
        ...
        case JVV_DTYPE_INTERNAL_OBJECT:
            ...
            if (vflags & JPR_FLAG_SHOW_OBJECTS) {
                did_cvt = 0;
                if (vflags & JPR_FLAG_TOSTRING) {
                    jstat = jpr_int_object_tostring((struct jrunexec *)jx, prbuf, prlen, prmax, jvv, &did_cvt, vflags);
                }
                if (!did_cvt) {
#if IS_DEBUG
                    append_charval(prbuf, prlen, prmax, "[");
        ...
        case JVV_DTYPE_ARRAY:
            ...
            jstat = jpr_jvarvalue_array_tostring((struct jrunexec *)jx, prbuf, prlen, prmax, jvv->jvv_val.jvv_val_array, vflags);
            break;
        ...
        case JVV_DTYPE_OBJPTR:
            ...
            switch (jvv->jvv_val.jvv_objptr.jvvp_dtype) {
                case JVV_DTYPE_ARRAY   :
                    ...
                    jstat = jpr_jvarvalue_array_tostring((struct jrunexec *)jx, prbuf, prlen, prmax,
            ...
            switch (jvv->jvv_val.jvv_objptr.jvvp_pval->jvv_dtype) {
                case JVV_DTYPE_ARRAY   :
                    ...
                    jstat = jpr_jvarvalue_array_tostring((struct jrunexec *)jx, prbuf, prlen, prmax,
                        jvv->jvv_val.jvv_objptr.jvvp_pval->jvv_val.jvv_val_array, vflags);
                    break;
                default:
                    append_printf(prbuf, prlen, prmax, "*Unknown object pointer type=%d*",
                        jvv->jvv_val.jvv_objptr.jvvp_pval->jvv_dtype);
                    break;
            }
#endif
            break;
        ...
        case JVV_DTYPE_DYNAMIC:
            ...
            if (vflags & JPR_FLAG_SHOW_LVALUE) {
                jstat = jexp_eval_rval((struct jrunexec *)jx, jvv, &jvvtmp);
                if (!jstat) {
                    jstat = jpr_jvarvalue_tostring((struct jrunexec *)jx, prbuf, prlen, prmax, jvvtmp, vflags);
                }
            }
            break;
...
*/
    ...
    jstat = jpr_jvarvalue_tostring(jx, charstar, &cslen, &csmax, jvv, vflags);
...
*/
    ...
    vtyp = jpr_get_vartype(jx, jvv);
    if (vtyp & show_flags) matches = 1;
        ...
        if (pvix) {
            if ((*pvix) < 0 || (*pvix) >= jcx->jcx_njvv) {
                printf("**** variable index %d is out of range[0-%d]",
                    (*pvix), jcx->jcx_njvv);
            } else if (!jpr_vartype_matches(jx, jcx->jcx_ajvv + (*pvix), show_flags)) {
                /* do nothing */
            } else {
...
int jpr_exec_debug_show_allvars(
                    struct jrunexec * jx, int show_flags)
{
/*
** 02/22/2021
*/
    ...
    if (!jcx) {
        ...
        while (!jstat && jcx) {
            jstat = jpr_debug_show_jcontext(jx, jcx, 1, JPR_FLAGS_ALL_TYPES | JPR_FLAG_SHOW_TYPE, show_flags);
...
*/
    ...
    while (ix >= 0) {
        ...
        jcx = jxfs->jfs_jcx;
        jvvf = jxfs->jfs_jvvf;
        append_function_args((struct jrunexec *)jx, &argbuf, &arglen, &argmax, jvvf, jcx, 1);
...
*/
    ...
    if (!strcmp(toke, "allclasses")) {
        jstat = jpr_exec_debug_show_allvars(jx, SHOWVAR_FLAG_CLASSES);
    } else if (!strcmp(toke, "allvars")) {
        jstat = jpr_exec_debug_show_allvars(jx, SHOWVAR_FLAG_VARS);

...
/*                                                             */
/* 01/31/2022                                                  */
/*      MS Dev Debug Properties                                */
/*          Command             : $(TargetPath)                */
/*          Command Arguments   : -pause -parms parms.txt      */
/*          Working Directory   : $(ProjectDir)\Run            */
/***************************************************************/
...
**      BNF: https://tomcopeland.blogs.com/EcmaScript.html
**
...
int glb_jsflags_to_jenflags(int gjsflags)
{
    ...
    if (gjsflags & GLB_JSFLAG_DEBUGFEATURES) {
        jenflags |= JENFLAG_DEBUGFEATURES;
    }
...
*/
    ...
    if (!glb->glb_jse) {
        ...
        jstat = jenv_initialize(glb->glb_jse);
    }

    if (!jstat) {
        jstat = jenv_xeq_string(glb->glb_jse, jsstr);
    }

    if (jstat) {
        if (jstat == JERR_EXIT) {
            jstat = 0;
        } else if (jstat == JERR_RETURN) {
            jstat = 0;
        } else {
            jerrmsg = jenv_get_errmsg(glb->glb_jse, 0, jstat, &jerrnum);
            estat = set_error(ESTAT_EXECUTE_STRING,
                "Error executing string: %s", jerrmsg);
            Free(jerrmsg);
        }
    }
...
*/
    ...
    /* Add null terminator */
    if ((*filestringlength) == fstrmax) {
        fstrmax += 2;
        (*filestring) = Realloc((*filestring), char, fstrmax);
    }
    (*filestring)[*filestringlength] = '\0';
...
int getargs(int argc, char *argv[], struct globals * glb)
{
    ...
    for (ii = 1; !estat && ii < argc; ii++) {
        if (argv[ii][0] == '-') {
            ...
            } else if (!Stricmp(argv[ii], "-debugfeatures")) {
                glb->glb_jsflags |= GLB_JSFLAG_DEBUGFEATURES;

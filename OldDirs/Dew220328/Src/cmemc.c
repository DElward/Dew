...
/*
** 08/08/2020 - Setup INTR as an integer to same size as a void*
**              **** Needs works for non-Windows platforms
*/
...
#if CMEM_TYPE == 2
    #define MEM_CLEAR   1   /* Set memory to '~' after freeing */
#else
    #define MEM_CLEAR   0
#endif
...
/***************************************************************/
#if CMEM_PRINTF
#define CMEMDBG_OUTFILE "DewMem.txt"
void dbgprintf(char *fmt, ...) {

    va_list args;
    char * msgptr;
    static FILE * debug_outf = NULL;

    va_start (args, fmt);
    msgptr = Vsmprintf (fmt, args);
    va_end (args);

    if (!debug_outf) {
        debug_outf = fopen(CMEMDBG_OUTFILE, "w");
        if (!debug_outf) {
            printf("dbgprintf() Cannot open file: %s\n", CMEMDBG_OUTFILE);
            debug_outf = stderr;
        }
    }

    fputs(msgptr, debug_outf);
...
#define hash(value) (int)(ABS(value) % (hashtable->hashsize))
...
/*@*/static void checkmeminfo(void* mi) {
    ...
    if (!meminfo || !meminfo->ptr) {                          /* 03/18/2021 */
        printf("   Null pointer created by '%s', line %d\n",  /* 03/18/2021 */
               meminfo->fname, meminfo->linenum);             /* 03/18/2021 */
        return;                                               /* 03/18/2021 */
    }                                                         /* 03/18/2021 */
...
*/
    ...
    if (!ptr) {
        ...
        printf("New(%ld,%ld) at %s, line %d\n",
            (long)itemsize, (long)numitems, fname, linenum);
        printf("Bytes in heap=%ld\n", totbytes);
        show_mallinfo (stdout);
        *((char*)0) = 0;  /* Cause blowout */
        exit(1);
...
printf("******************************************************************\n");
    ...
    if (!hashrec)
        ...
        meminfo = hashrec->blockptr;
#if CMEM_PRINTF
    dbgprintf("Realloc :\t%4d\t%4d\t%4d\n",
            (long)numbytes - meminfo->numbytes, numbytes, meminfo->numbytes);
#endif
         if (rptr == ptr) {
          meminfo->numresame++;
          totresame++;
...
/*  fprintf(repfnum, "ptr=%8lx", meminfo->ptr); */
    sprintf(pbuf, "0x%lx", (long)(INTR)meminfo->ptr);
    get_abuf(abuf, TMIN(14,meminfo->numbytes), (void*)(meminfo->ptr));
...
char * malloced_by(void * ptr, char * buf) {
    ...
    if (!ptr) {
        ...
        if (!hashrec) {
            ...
            if (!meminfo) {
                sprintf(buf, "Pointer (0x%lx) not malloc'd", (long)(INTR)ptr);
            } else {
                sprintf(buf,
      "Pointer (0x%lx) is byte %ld of %ld bytes malloc'd by %s line %ld",
                (long)(INTR)ptr, (long)((INTR)ptr - meminfo->ptr), meminfo->numbytes,
                meminfo->fname, meminfo->linenum);
            }
        } else {
            meminfo = hashrec->blockptr;
            sprintf(buf, "Pointer (0x%lx) malloc'd by %s line %ld",
                (long)(INTR)ptr, meminfo->fname, meminfo->linenum);
        }
    }

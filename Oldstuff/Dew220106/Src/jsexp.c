...
//  #endif
    ...
    while (!XSTAT_IS_RETURNABLE(jx)) {
        jstat2 = jrun_pop_xstat(jx);
    }

    //jvar_pop_vars(jx);

    jstat2 = jrun_pop_jfuncstate(jx);

    jstat2 = jrun_update_xstat(jx);

    if (!is_eof) jstat2 = jrun_next_token(jx, pjtok);
...
** 01/19/2021
**
** See also: jprep_eval_function_call()
*/
    int jstat;
...
/***************************************************************/
#if DEBUG_OLD_ASSIGN
int xxxjexp_eval_assignment_inactive(
                    struct jrunexec * jx,
                    struct jtoken ** pjtok)
{
/*

...
*/
    ...
    if (!jstat) {
        ...
        jstat = jvar_new_local_function(jx, jti, &fjvv, jcxflags, skip_flags);
    }
    ...
    switch ((*pjtok)->jtok_typ) {
        ...
        case JSW_KW:
            if ((*pjtok)->jtok_kw == JSKW_FUNCTION) {
                jstat = jrun_next_token(jx, pjtok);
                if (!jstat) jstat = jprep_function_primary(jx, pjtok);
            } else {
                jstat = jrun_next_token(jx, pjtok);
            }
            break;
...
*/
    ...
    while (!jstat && !done) {
        if (!jvar_is_valid_varname(jx, (*pjtok))) {
            pvix = jvar_find_in_jvarrec(jti->jti_jvar, (*pjtok)->jtok_text);
            if (!pvix && (skip_flags & SKIP_FLAG_HOIST)) {
                vix = jvar_insert_into_jvarrec(jti->jti_jvar, (*pjtok)->jtok_text);
            }
            jstat = jrun_next_token(jx, pjtok); /* delimiter or '=' */
            if ((*pjtok)->jtok_kw == JSPU_EQ) {
                jstat = jrun_next_token(jx, pjtok);
...
*/
    ...
    jstat = jprep_newvar_stmt(jx, jti, pjtok, JCX_FLAG_CONST, skip_flags);
...
*/
    ...
    jstat = jprep_newvar_stmt(jx, jti, pjtok, 0, skip_flags);
...
*/
    ...
    jti = jtok->jtok_jti;
    if (!jti) {
        jti = jtok_new_jtokeninfo();
        jti->jti_jvar = jvar_new_jvarrec();
        INCVARRECREFS(jti->jti_jvar);
        jtok->jtok_jti = jti;
    }
    ...
    while (!jstat && !done) {
        ...
        switch ((*pjtok)->jtok_kw) {
            ...
            case JSKW_CONST:
                jstat = jrun_next_token(jx, pjtok);
                if (!jstat && braces) {
                    jti = jprep_ensure_jtokeninfo(jx, jtok_stack[braces - 1]);
                    jstat = jprep_const_stmt(jx, jti, pjtok, skip_flags);
                }
                break;
            ...
            case JSKW_FUNCTION:
                ...
                if (!jstat && braces) {
                    jti = jprep_ensure_jtokeninfo(jx, jtok_stack[braces - 1]);
                    jstat = jprep_function_stmt(jx, jti, pjtok, skip_flags);
                }
                break;
            ...
            case JSKW_LET:
                ...
                if (!jstat && braces) {
                    jti = jprep_ensure_jtokeninfo(jx, jtok_stack[braces - 1]);
                    jstat = jprep_let_stmt(jx, jti, pjtok, skip_flags);
                }
                break;

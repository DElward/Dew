...
*/
    ...
    **      var val=subs(3, 8);
    */
    if (jvv->jvv_dtype == JVV_DTYPE_FUNCVAR) {
        jvar_free_jvarvalue_data(jvvtgt);
    ...
    } else if (jvv->jvv_dtype == JVV_DTYPE_IMETHVAR) {
        jvar_free_jvarvalue_data(jvvtgt);
...
#endif
        jstat = jvar_store_jvarvalue(jx, vjvv, ajvv2);
    ...
    switch ((*pjtok)->jtok_typ) {
        ...
        case JSW_PU:
            switch ((*pjtok)->jtok_kw) {
                ...
                default:
                    jstat = jrun_set_error(jx, JSERR_UNEXPECTED_PUNCTUATOR,
...
/***************************************************************/
static int jexp_binop_jvv_dot(struct jrunexec * jx,
    struct jvarvalue * jvv,
    void * this_ptr,
    struct jvarvalue * ajvv1,
    struct jvarvalue * ajvv2)
...
*/
    ...
    if (tix < 0 || tix >= JVV_NUM_DTYPES) {
        ...
        if (!jvv) {
            jstat = jrun_set_error(jx, JSERR_INVALID_TYPE,
                "Type has no elements. Type=%s", jvar_get_dtype_name(dtype));
        } else {
            jstat = jexp_binop_jvv_dot(jx, jvv, this_ptr, ajvv1, ajvv2);
        }
    }
    ...
    switch (cjvv->jvv_dtype) {
        ...
        case JVV_DTYPE_INTERNAL_OBJECT   :
            ...
            if (!mjvv) {
                ...
                ajvv1->jvv_val.jvv_int_method = mjvv->jvv_val.jvv_int_method;
                INCIMETHREFS(mjvv->jvv_val.jvv_int_method);
                //ajvv1->jvv_dtype = JVV_DTYPE_FUNCVAR;
                //ajvv1->jvv_val.jvv_val_funcvar.jvvfv_jvvf = jvvsrc->jvv_val.jvv_val_function;
                //ajvv1->jvv_val.jvv_val_funcvar.jvvfv_var_jcx = var_jcx;
                //INCFUNCREFS(jvvsrc->jvv_val.jvv_val_function);
#endif
            } else {
                jstat = jrun_set_error(jx, JSERR_UNSUPPORTED_OPERATION,

...
*/
    ...
    if ((*pjtok)->jtok_typ == JSW_STRING) {
        ...
        if (!jstat) {
            ...
            jstat = jrun_set_error(jx, JSERR_OBJ_TOKEN_TYPE,
    ...
    if (!jstat) {
        if ((*pjtok)->jtok_kw != JSPU_COLON ) {
            jstat = jrun_set_error(jx, JSERR_OBJ_EXP_COLON,
...
*/
    ...
    if (!jstat) {
        if ((*pjtok)->jtok_kw == JSPU_CLOSE_BRACE ) {
            ...
            jstat = jrun_set_error(jx, JSERR_EXP_CLOSE_BRACE,
...
*/
    ...
    jstat = job_dup_jvarvalue_object(jx, &(jvvtgt->jvv_val.jvv_jvvb), jvvsrc->jvv_val.jvv_jvvb);
    if (!jstat) {

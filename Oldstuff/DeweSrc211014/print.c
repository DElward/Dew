    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_LVAL:
            ...
            if (vflags & JPR_FLAG_SHOW_LVALUE) {
                jstat = jpr_jvarvalue_tostring(jx, prbuf, prlen, prmax, jvv->jvv_val.jvv_lval.jvvv_lval, vflags);
            }
            break;
        ...
        case JVV_DTYPE_ARRAY:
            if (vflags & JPR_FLAG_SHOW_TYPE) append_charval(prbuf, prlen, prmax, "ARRAY: ");
            if (vflags & JPR_FLAG_LOG) {
                append_printf(prbuf, prlen, prmax, "(%d) ", jvv->jvv_val.jvv_val_array->jvva_nvals);
            }
            append_charval(prbuf, prlen, prmax, "[");
            for (ii = 0; ii < jvv->jvv_val.jvv_val_array->jvva_nvals; ii++) {
                if (ii) append_charval(prbuf, prlen, prmax, ", ");
                prbuf2 = NULL;
                prlen2 = 0;
                prmax2 = 0;
                jstat = jpr_jvarvalue_tostring(jx, &prbuf2, &prlen2, &prmax2,
                    &(jvv->jvv_val.jvv_val_array->jvva_avals[ii]), vflags | JPR_FLAGS_SHOW_QUOTES);
                if (!jstat) {
...
/***************************************************************/
int jpr_exec_debug_show_runstack(
                    struct jrunexec * jx)
{
/*
** 09/20/2021
*/
    ...
    for (sp = jx->jx_njs - 1; sp >= 0; sp--) {
        ...
        if (jx->jx_js[sp]->jxs_jcx) {
            printf("    %d. %-20s %-20s 0x%08x\n", sp, xbuf, fbuf, (int)jx->jx_js[sp]->jxs_jcx);
        } else {
            printf("    %d. %-20s %-20s\n", sp, xbuf, fbuf);
        }
    }

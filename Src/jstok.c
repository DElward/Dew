...
{
    if (jtok->jtok_jti) jtok_free_jtokeninfo(jtok->jtok_jti);
    Free(jtok->jtok_text);
    Free(jtok);
}
/***************************************************************/
...
*/
    ...
    while (!done) {
        if (JTOK_CURRENT_CHAR(jstr, jstrix) != 'u') {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_ID_INVALID_ESCAPE,
    ...
    if (jstat) {
        ...
        if (jtok->jtok_tlen == 0 || !JTOK_FIRST_ID_CHAR(jtok->jtok_text[0])) {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_INVALID_ID,
                    "Invalid identifer");
        } else if (jrun_find_keyword(jtok->jtok_text)) {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_ID_RESERVED,
                    "Identifer reserved");
        } else {
...
/***************************************************************/
void jtok_add_string_to_jtokenlist(struct jrunexec * jx,
    struct jtokenlist * jtl,
    const char * jstr)
{
/*
...
*/
    ...
    if (!JTOK_VAR_CHAR(jstr[0]) && !JTOK_VAR_CHAR(jstr[0])) {
        jtok = jtok_new_jtoken();
        jtok_append_token(jtok, jstr[0]);
        jstrix = 0;
        jstat = jtok_get_punctuator(jx, js_punctuator_list, jtok, jstr, &jstrix);
...
/***************************************************************/
struct jtifuncoper * jrun_new_jtifuncoper(void)
{
/*
** 01/29/2022
*/
    struct jtifuncoper * jtifo;

    jtifo = New(struct jtifuncoper, 1);
    INIT_JVARVALUE(&(jtifo->jtifo_fjvv));
    jrun_init_jxcursor(&(jtifo->jtifo_end_jxc));

    return (jtifo);
}
/***************************************************************/
void jrun_free_jtifuncoper(struct jtifuncoper * jtifo)
{
/*
** 01/29/2022
*/
    if (jtifo) {
        jvar_free_jvarvalue_data(&(jtifo->jtifo_fjvv));
        Free(jtifo);
    }
}
/***************************************************************/
struct jtiopenbrace * jrun_new_jtiopenbrace(void)
{
/*
** 01/29/2022
*/
    ...
    jtiob = New(struct jtiopenbrace, 1);
    jtiob->jtiob_jvar = jvar_new_jvarrec();
    INCVARRECREFS(jtiob->jtiob_jvar);
    jrun_init_jxcursor(&(jtiob->jtiob_end_jxc));

    return (jtiob);
}
/***************************************************************/
void jrun_free_jtiopenbrace(struct jtiopenbrace * jtiob)
{
/*
** 01/29/2022
*/
    if (jtiob) {
        if (jtiob->jtiob_jvar) jvar_free_jvarrec(jtiob->jtiob_jvar);
        Free(jtiob);
    }
}
/***************************************************************/
...
*/
    ...
    switch (jti->jti_type) {
        case JTI_TYPE_OPEN_BRACE:
            /* if (jti->jti_val.jtibr_jvar) jvar_free_jvarrec(jti->jti_val.jtibr_jvar); */
            if (jti->jti_val.jti_val_jtiob) jrun_free_jtiopenbrace(jti->jti_val.jti_val_jtiob);
            break;

        case JTI_TYPE_FUNCTION:
            if (jti->jti_val.jtifu_end_jxc) jrun_free_jxcursor(jti->jti_val.jtifu_end_jxc);
            break;

        case JTI_TYPE_FUNCTION_OPERAND:
            if (jti->jti_val.jtifo) jrun_free_jtifuncoper(jti->jti_val.jtifo);
            break;
...
*/
    ...
    jti = New(struct jtokeninfo, 1);
    jti->jti_type = titype;
    jti->jti_val.jti_none = NULL;
...
*/
    ...
    if (!jti) {
        ...
        switch (jti->jti_type) {
            case JTI_TYPE_OPEN_BRACE:
                /* jti->jti_val.jtibr_jvar = jvar_new_jvarrec();        */
                /* INCVARRECREFS(jti->jti_val.jtibr_jvar);              */
                jti->jti_val.jti_val_jtiob = jrun_new_jtiopenbrace();
                break;

            case JTI_TYPE_FUNCTION:
                jti->jti_val.jtifu_end_jxc = jrun_new_jxcursor();
                break;

            case JTI_TYPE_FUNCTION_OPERAND:
                jti->jti_val.jtifo = jrun_new_jtifuncoper();
                break;

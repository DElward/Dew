...
*/
    ...
    if (slen <= tmax) {
        ...
        slen = tmax - 1;
        memcpy(tgt, src, slen);
        tgt[slen] = '\0';   /* 02/22/2022: Was tgt[tmax]. Fixed to tgt[slen] */
    }
}
/***************************************************************/
void strmcat(char * tgt, const char * src, size_t tmax) {
/*
** Copies src to tgt, trucating src if too long.
**  tgt always termintated with '\0'
*/
    ...
    if (tlen + slen <= tmax) {
        ...
        slen = tmax - (tlen + 1);
        if (slen > 0) memcpy(tgt + tlen, src, slen);
        tgt[tmax - 1] = '\0';
    }
}
/***************************************************************/
...
**
*/
    append_chars(cstr, slen, smax, val, (int)strlen(val));
}
/***************************************************************/
...
**
** 05/09/2022 - Copied to Warehouse.
**
**      **** Any fixes should be propagated to Warehouse
*/
    int cstat;
    int neg;
    int ix;

...
int jexp_eval_function_call_inactive(
                    struct jrunexec * jx,
                    struct jtoken ** pjtok);
...
/***************************************************************/
#if 0
static void xxjvar_store_jvarvalue(
    struct jvarvalue * jvvtgt,
...
*/
    if (jvvsrc->jvv_dtype == JVV_DTYPE_FUNCTION) {
        ...
        jvvtgt->jvv_val.jvv_lval.jvvv_var_jcx = var_jcx;
    }
}
/***************************************************************/
...
*/
    ...
    jstat = jpr_jvarvalue_tostring(jx, &prbuf, &prlen, &prmax, jvvsrc, JPR_FLAGS_TO_STRING_TYPES);
    if (!jstat) {
...
*/
    ...
    jstat = jexp_eval_rval(jx,ajvv1, &jvv1);
...
*/
    ...
    jstat = jexp_eval_rval(jx,ajvv1, &jvv1);
    if (jstat) return (jstat);
    jstat = jexp_eval_rval(jx,ajvv2, &jvv2);
    if (jstat) return (jstat);
...
*/
    ...
    switch (jvv1->jvv_dtype) {
        ...
        case JVV_DTYPE_CHARS:
            ...
            jvar_free_jvarvalue_data(jvv1);     /* 07/15/2021 */
            jvar_free_jvarvalue_data(jvv2);     /* 07/15/2021 */
            break;
        ...
        case JVV_DTYPE_OBJPTR:
            switch (jvv1->jvv_val.jvv_objptr.jvvp_dtype) {
                case JVV_DTYPE_ARRAY   :
                    if (jvv2->jvv_dtype == JVV_DTYPE_OBJPTR &&
                        jvv2->jvv_val.jvv_objptr.jvvp_dtype == JVV_DTYPE_ARRAY) {
                        if (jvv1->jvv_val.jvv_objptr.jvvp_val.jvvp_val_array == /* pointer compare */
                            jvv2->jvv_val.jvv_objptr.jvvp_val.jvvp_val_array) {
                            (*cmpval) = 0;
                        } else {
                            (*cmpval) = 1;
                        }
...
/***************************************************************/
int jvar_do_assign_by_reference(
    struct jrunexec * jx,
    struct jvarvalue * jvvtgt,
    struct jvarvalue * jvv)
{
    ...
    switch (jvv->jvv_dtype) {
        case JVV_DTYPE_FUNCVAR:
            ...
            */
            ...
            jvvtgt->jvv_val.jvv_val_funcvar.jvvfv_var_jcx = NULL;
        ...
        case JVV_DTYPE_IMETHVAR:
            ...
            jvvtgt->jvv_val.jvv_val_imethvar.jvvimv_this_ptr = NULL;
...
/***************************************************************/
int jvar_is_assign_by_reference(struct jrunexec * jx, struct jvarvalue * jvvsrc)
{
/*
** 10/19/2021
*/
...
*/
    ...
    if (jvar_is_assign_by_reference(jx, jvv)) {
        jvar_free_jvarvalue_data(jvvtgt);

        //jstat = jvar_do_assign_by_reference(jx, jvvtgt, jvv);
...
*/
    ...
    if (ajvv1->jvv_dtype == JVV_DTYPE_LVAL) {
        vjvv = ajvv1->jvv_val.jvv_lval.jvvv_lval;
        /*
        ** The following is needed to prevent the following from working:
        **      var abc = "Abcdefghijklmn";
        **      var subs = abc.substring;
        **      var val=subs(3, 8);
        */
        if (vjvv->jvv_dtype == JVV_DTYPE_FUNCVAR) {
            vjvv->jvv_val.jvv_val_funcvar.jvvfv_this_obj = NULL;
        } else if (vjvv->jvv_dtype == JVV_DTYPE_IMETHVAR) {
            vjvv->jvv_val.jvv_val_imethvar.jvvimv_this_ptr = NULL;
        }
        jstat = jvar_assign_jvarvalue(jx, vjvv, ajvv2);
        if (!jstat) jvar_free_jvarvalue_data(ajvv2);
    } else if (ajvv1->jvv_dtype == JVV_DTYPE_DYNAMIC) {
        if (!ajvv1->jvv_val.jvv_val_dynamic.jvvy_set_proc) {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_PROP_READ_ONLY,
                "Property is read-only and may not be set");
        } else {
            jstat = (ajvv1->jvv_val.jvv_val_dynamic.jvvy_set_proc)(jx,
                ajvv1->jvv_val.jvv_val_dynamic.jvvy_this_ptr,
                ajvv1->jvv_val.jvv_val_dynamic.jvvy_arg,
                ajvv2);
        }
    } else {
...
*/
    ...
    if ((*pjtok)->jtok_kw == JSPU_OPEN_BRACE) {
        ...
        jstat = jprep_preprocess_block(jx, jvvf, pjtok, SKIP_FLAG_HOIST);
    } else {
        jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERROBJ_SCRIPT_SYNTAX_ERROR,
...
*/
    ...
    jstat = jvar_parse_function(jx, pjtok, jvv);
    if (jstat) {
        jvv_free_jvarvalue(jvv);
    } else {
        if (jvv->jvv_dtype != JVV_DTYPE_FUNCTION ||
            !jvv->jvv_val.jvv_val_function->jvvf_func_name) {
            jstat = jrun_set_error(jx, JSERROBJ_SCRIPT_SYNTAX_ERROR,
                "Expecting function name.");
            jvv_free_jvarvalue(jvv);
        } else {
            //jstat = jvar_new_local_function(jx,
            //    jvv->jvv_val.jvv_val_function->jvvf_func_name, jvv);
        }
    }
...
#endif
/***************************************************************/
static int jvar_parse_function_arguments_inactive(
                    struct jrunexec * jx,
                    struct jtoken ** pjtok)
{
/*
...
*/
    ...
    if (!jstat) {
        jstat = jprep_skip_generic_statement(jx, pjtok, 0);
    }
...
*/
    ...
    if (jvv->jvv_dtype == JVV_DTYPE_LVAL) {
        ...
        jstat = (jvv->jvv_val.jvv_val_dynamic.jvvy_get_proc)(jx,
            jvv->jvv_val.jvv_val_dynamic.jvvy_this_ptr,
            jvv->jvv_val.jvv_val_dynamic.jvvy_arg,
            prtnjvv);
    } else {
        (*prtnjvv) = jvv;
    }
...
*/
    ...
    switch ((*pjtok)->jtok_typ) {
        ...
        case JSW_KW:
            switch ((*pjtok)->jtok_kw) {
                ...
                case JSKW_FUNCTION:
                    jstat = jrun_next_token(jx, pjtok);
                    INIT_JVARVALUE(&tjvv);
                    if (!jstat) jstat = jvar_parse_function(jx, pjtok, &tjvv);
                    if (!jstat && tjvv.jvv_dtype == JVV_DTYPE_FUNCTION) {
                        //fjvv->jvv_val.jvv_val_function->jvvf_flags |= JVVF_FLAG_FREE_FUNC;
                        jvv->jvv_dtype = JVV_DTYPE_FUNCVAR;
                        jvv->jvv_val.jvv_val_funcvar.jvvfv_jvvf = tjvv.jvv_val.jvv_val_function;
                        //jvv->jvv_val.jvv_val_funcvar.jvvfv_var_jcx = jvar_get_current_jcontext(jx);
        ...
        case JSW_ID:
            jcxflags = 0;
            vjvv = jvar_find_variable(jx, (*pjtok), &var_jcx, &jcxflags);
            if (vjvv) {
    ...
    switch ((*pjtok)->jtok_typ) {
        ...
        case JSW_PU:
            switch ((*pjtok)->jtok_kw) {
                ...
                default:
                    jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_UNEXPECTED_PUNCTUATOR,
                        "Unexpected inactive punctuator: %s", (*pjtok)->jtok_text);
                    break;
            }
            break;
...
//  #endif
    ...
    if ((jstat == JERR_RETURN) || (jstat == JERR_EXIT)) {
        ...
        jstat = 0;
        vjvv = jrun_get_return_jvv(jx);
        if (vjvv->jvv_dtype == JVV_DTYPE_LVAL) {
            COPY_JVARVALUE(rtnjvv, vjvv->jvv_val.jvv_lval.jvvv_lval);
        } else {
    ...
    if (!jstat) {
        while (!XSTAT_IS_RETURNABLE(jx)) { jstat = jrun_pop_xstat(jx); }
    }
    ...
    if (!jstat && !is_eof) jstat = jrun_next_token(jx, pjtok);
...
#endif
/***************************************************************/
static int jexp_internal_method_call(
                    struct jrunexec * jx,
                    struct jvarvalue_int_method * imeth,
                    struct jvarvalue * this_ptr,
...
*/
    ...
    if (mjvv->jvv_dtype == JVV_DTYPE_LVAL) {
        mjvv = mjvv->jvv_val.jvv_lval.jvvv_lval;
        jcx = mjvv->jvv_val.jvv_lval.jvvv_var_jcx;
    }
...
*/
    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_INTERNAL_OBJECT   :
            mjvv = jvar_int_object_member(jx, jvv, method_name, &injcxflags);
            if (!mjvv) {
                jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_UNDEFINED_CLASS_MEMBER,
                    "Undefined class member: %s", method_name);
            } else if (mjvv->jvv_dtype == JVV_DTYPE_INTERNAL_METHOD) {
                if (injcxflags & (JCX_FLAG_PROPERTY | JCX_FLAG_OPERATION)) {
                    jstat = jexp_internal_method_call(jx, mjvv->jvv_val.jvv_int_method, this_ptr, ajvv1, rtnjvv);
                    if (!jstat) {
                    ...
                    ajvv1->jvv_val.jvv_val_imethvar.jvvimv_this_ptr = this_ptr;
...
*/
    ...
    jstat = jexp_eval_rval(jx, mjvv, &cjvv);
    if (jstat) return (jstat);
        ...
        switch (cjvv->jvv_val.jvv_objptr.jvvp_dtype) {
            case JVV_DTYPE_ARRAY   :
                jstat = jexp_binop_type_bracket(jx, JVV_DTYPE_ARRAY,
                    cjvv->jvv_val.jvv_objptr.jvvp_val.jvvp_val_array, jvvparms, rtnjvv);
                break;
            default:
                jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_INVALID_STORE,
                    "Illegal object pointer compare %s",
                    jvar_get_dtype_name(cjvv->jvv_val.jvv_objptr.jvvp_dtype));
                break;
        }
    } else {
        jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_INVALID_TYPE,
...
/***************************************************************/
#ifdef OLD_WAY
static int xxjexp_binop_dot(struct jrunexec * jx,
    struct jvarvalue * ajvv1,
    struct jvarvalue * ajvv2)
{
/*
...
*/
    ...
    jstat = jexp_eval_rval(jx,ajvv1, &cjvv);
    if (jstat) return (jstat);
    ...
    switch (cjvv->jvv_dtype) {
        ...
        case JVV_DTYPE_INTERNAL_OBJECT   :
            mjvv = jvar_int_object_member(jx, cjvv, ajvv2->jvv_val.jvv_val_token.jvvt_token, &injcxflags);
            if (!mjvv) {
                jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_UNDEFINED_CLASS_MEMBER,
                    "Undefined class member: %s",
                    ajvv2->jvv_val.jvv_val_token.jvvt_token);
...
*/
    ...
    switch (jvv->jvv_dtype) {
        ...
        case JVV_DTYPE_JSFLOAT   :
            ...
            break;
        case JVV_DTYPE_CHARS   :
            if (jvv->jvv_val.jvv_val_chars.jvvc_length + 2 < valstrsz) {
                valstr[0] = '\"';
                memcpy(valstr + 1, jvv->jvv_val.jvv_val_chars.jvvc_val_chars,
                               jvv->jvv_val.jvv_val_chars.jvvc_length);
                valstr[jvv->jvv_val.jvv_val_chars.jvvc_length + 1] = '\"';
                valstr[jvv->jvv_val.jvv_val_chars.jvvc_length + 2] = '\0';
            } else {
                valstr[0] = '\"';
        ...
        case JVV_DTYPE_LVAL:
            ...
            strmcpy(valstr, nbuf, valstrsz);
            jexp_quick_tostring(jx, jvv->jvv_val.jvv_lval.jvvv_lval, nbuf, sizeof(nbuf));
            strmcat(valstr, nbuf, valstrsz);
            break;
        case JVV_DTYPE_INTERNAL_CLASS:
        ...
        case JVV_DTYPE_FUNCTION:
            sprintf(nbuf, "function ");
            strmcpy(valstr, nbuf, valstrsz);
            strmcat(valstr, jvv->jvv_val.jvv_val_function->jvvf_func_name, valstrsz);
            break;
        case JVV_DTYPE_ARRAY:
            strmcpy(valstr, "[", valstrsz);
            tlen = 0;
            for (ii = 0; ii < jvv->jvv_val.jvv_val_array->jvva_nvals; ii++) {
                if (ii && tlen < valstrsz) valstr[tlen++] = ',';
                jexp_quick_tostring(jx, jvv->jvv_val.jvv_val_array->jvva_avals[ii], nbuf, sizeof(nbuf));
                nlen = IStrlen(nbuf);
                if (nlen >= (valstrsz - tlen)) nlen = valstrsz - tlen - 1;
                if (nlen > 0) {
                    memcpy(valstr + tlen, nbuf, nlen);
                    tlen += nlen;
                    valstr[tlen] = '\0';
                }
            }
...
*/
    ...
    jstat = jpr_jvarvalue_tostring(jx, &prbuf, &prlen, &prmax, jvv, JPR_FLAGS_ALL_TYPES);
    if (jstat) {
...
*/
    ...
    jstat = jexp_eval_stack(jx, xstack, depth, JX_DOT_PREC, JXE_OPFLAG_FUNCTION_CALL);
    if (!jstat) jstat = jrun_next_token(jx, pjtok);
...
*/
    ...
    jstat = jexp_eval_stack(jx, xstack, depth, JX_DOT_PREC, JXE_OPFLAG_OPEN_BRACKET);
    if (!jstat) jstat = jrun_next_token(jx, pjtok);
    INIT_JVARVALUE(&jvvparms);
    if (!jstat) {
        jstat = jexp_eval(jx, pjtok, &jvvparms);
                ...
                if (xlx >= 0 && (jexp_exp_list[xlx].jxe_opflags & JXE_OPFLAG_UNARY_OPERATION)) {
                    ...
                    if ((*pjtok)->jtok_typ == JSW_PU) {
                        if ((*pjtok)->jtok_kw == JSPU_OPEN_PAREN) {
                            /* Function call */
                            jstat = jexp_eval_function_call(jx, pjtok, xstack, &depth);
                        } else if ((*pjtok)->jtok_kw == JSPU_OPEN_BRACKET) {
...
/***************************************************************/
static int jvar_add_new_ad_hoc_local_variable(
                    struct jrunexec * jx,
                    struct jtoken ** pjtok,
...
*/
    ...
    if (!jstat) {
        ...
        if (!jcx) {
            jstat = jrun_set_error(jx, errtyp_UnimplementedError, JSERR_INTERNAL_ERROR,
                "Internal error: No current context.");
            return (jstat);
        }
#endif
        jstat = jvar_new_ad_hoc_variable(jx, jcx,
            (*pjtok)->jtok_text, vjvv, jcxflags);
        if (!jstat) (*pjvv) = vjvv;
    }
        ...
        if ((*pjtok)->jtok_typ == JSW_ID && next_jtok.jtok_kw == JSPU_EQ) {
            ...
            if (!vjvv) {
                if (jrun_get_strict_mode(jx)) {
                    ...
                    jstat = jvar_add_new_ad_hoc_local_variable(jx, pjtok, &vjvv);
                }
            }
            if (!jstat) jstat = jrun_next_token(jx, pjtok); /* '=' */
            if (!jstat) jstat = jrun_next_token(jx, pjtok);
            ...
            if (!jstat) {
                jstat = jvar_store_jvarvalue(jx, vjvv, jvv);
                if (!jstat) jvar_free_jvarvalue_data(jvv);
            }
        } else {
            jstat = jexp_eval_value(jx, pjtok, jvv);
            if (!jstat && (*pjtok)->jtok_kw == JSPU_EQ) {
                if (jvv->jvv_dtype != JVV_DTYPE_LVAL) {
                    ...
                    vjvv = jvv->jvv_val.jvv_lval.jvvv_lval;
                    jstat = jrun_next_token(jx, pjtok); /* '=' */
                    if (!jstat) jstat = jrun_next_token(jx, pjtok);
                    if (!jstat) {
                        jstat = jexp_eval_value(jx, pjtok, jvv);

...
*/
    ...
    if (jvvo->jvvo_free_func && jvvo->jvvo_this_ptr) (jvvo->jvvo_free_func)(jvvo->jvvo_this_ptr);
    Free(jvvo);
}
/***************************************************************/
...
{
    /*printf("jvar_free_jvarrec() nRefs=%d\n", jvar->jvar_nRefs);*/
    DECVARRECREFS(jvar);
    if (!jvar->jvar_nRefs) {
...
*/
    ...
    if (cjvv) {
        if (cjvv->jvv_dtype == JVV_DTYPE_INTERNAL_OBJECT) {
            jcx = jvar_get_int_object_jcontext(jx, cjvv->jvv_val.jvv_jvvo);
            if (jcx) {

...
*/
    ...
    if (jvvsrc->jvv_dtype == JVV_DTYPE_FUNCTION) {
        ...
        jvvtgt->jvv_val.jvv_val_funcvar.jvvfv_jvv = jvvsrc;
...
/***************************************************************/
#if ! DEBUG_OLD_ASSIGN
static int jexp_binop_equal(struct jrunexec * jx,
    struct jvarvalue * ajvv1,
    struct jvarvalue * ajvv2)
{
/*
        ...
        case JSW_KW:
            switch ((*pjtok)->jtok_kw) {
                ...
                case JSKW_FUNCTION:
                    jstat = jrun_next_token(jx, pjtok);
#if FUNCVARS == 1
                    fjvv = jvar_new_jvarvalue();    // How do we free this?
                    if (!jstat) jstat = jvar_parse_function(jx, pjtok, fjvv);
                    if (!jstat && fjvv->jvv_dtype == JVV_DTYPE_FUNCTION) {
                        //fjvv->jvv_val.jvv_val_function->jvvf_flags |= JVVF_FLAG_FREE_FUNC;
                        DECFUNCREFS(fjvv->jvv_val.jvv_val_function);
                        jvar_store_lval(jx, jvv, fjvv, jvar_get_current_jcontext(jx));
                    }
#elif FUNCVARS == 2
                    INIT_JVARVALUE(&tjvv);
...
*/
    ...
    jstat = (fjvv->jvv_val.jvv_int_method.jvvim_method)
        (jx, fjvv->jvv_val.jvv_int_method.jvvim_method_name, jvvargs, rtnjvv);
...
#if FUNCVARS == 1
        fjvv = mjvv->jvv_val.jvv_val_funcvar.jvvfv_jvv;
        jcx = mjvv->jvv_val.jvv_val_funcvar.jvvfv_var_jcx;
        jstat = jexp_call_function(jx, pjtok, fjvv->jvv_val.jvv_val_function, jvvargs, jcx, rtnjvv);
#elif FUNCVARS == 2
        jvvf = mjvv->jvv_val.jvv_val_funcvar.jvvfv_jvvf;
        jcx = mjvv->jvv_val.jvv_val_funcvar.jvvfv_var_jcx;
        jstat = jexp_call_function(jx, pjtok, jvvf, jvvargs, jcx, rtnjvv);
#endif
    } else if (mjvv->jvv_dtype == JVV_DTYPE_INTERNAL_METHOD) {
        jstat = jexp_internal_method_call(jx, mjvv, jvvargs, rtnjvv);
    } else if (mjvv->jvv_dtype == JVV_DTYPE_FUNCTION) {
        if (!jcx) jcx = jvar_get_head_jcontext(jx);
        jstat = jexp_call_function(jx, pjtok, mjvv->jvv_val.jvv_val_function, jvvargs, jcx, rtnjvv);
...
#if ! DEBUG_OLD_ASSIGN
    { 1, 0, JSPU_EQ                ,  5, jexp_binop_equal   , NULL              },
#endif
    { 0, 0, 0                      ,  0, NULL               , NULL              }
};
